...
def merge_head_elements(
    *,
    ...
    # Split head blocks into individual elements first, but preserve formatting where possible
    # For head_variable and single-element JSX blocks, preserve as-is
    layout_elements = []
    for block in layout_head_jsx_blocks:
        elements = _split_head_block_into_elements(block)
        layout_elements.extend(elements)
    
    head_var_elements = list(head_variable)  # Head variable elements are already individual
